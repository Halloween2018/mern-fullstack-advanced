...
/* 和上例相同的实现，代码看起来简单易读，且不需要在每个 promise 上进行错误判断，只要最终的 catch() 就可以捕获所有的异常 */
  ...
  function testWithGenerator() {
      ...
      co(function*() {
        ...
        const docs = yield db.collection('emplyees').find({id: 1}).toArray();
        console.log('Result of find:',docs);
    ...
    function testWithAsync() {
        ...
        async.waterfall([
            next => {
                MongoClient.connect('mongo://localhost/playground', next);
            },
            (connection, next) => {
                db = connection;
